  ...
  useEffect(() => {
    ...
    const drawGame = () => {
      ...
      if (directionRef.current.x === 0 && directionRef.current.y === 0) {
        ...
        ctx.fillText("WALLS_BYPASSED: USE ARROWS", CANVAS_SIZE / 2, CANVAS_SIZE / 2 + 50);
      }
    };
    ...
    const gameUpdate = () => {
      ...
      // 🛠️ WALL WRAPPING LOGIC (The "Non-Reducing" Fix)
      let nextX = snakeRef.current[0].x + directionRef.current.x;
      let nextY = snakeRef.current[0].y + directionRef.current.y;

      // If head goes out of bounds, wrap it to the other side
  ...
  return (
    <div className="flex flex-col items-center bg-zinc-950 p-4 sm:p-8 rounded-[2rem] border border-blue-500/20 shadow-2xl max-w-full">
      <div className="w-full flex justify-between mb-4 font-mono text-[10px] text-blue-400 uppercase">
        <span>Tunneling_Protocol // Active</span>
      ...
      <div className="relative p-1 bg-blue-500/10 rounded-xl border border-white/5">
        <canvas ref={canvasRef} width={CANVAS_SIZE} height={CANVAS_SIZE} className="bg-black rounded-lg block" />
      </div>

      <div className="mt-8 grid grid-cols-3 gap-3 sm:hidden">
        <div />
        <button onClick={() => changeDirection('UP')} className="h-14 w-14 bg-zinc-900 rounded-xl border border-blue-500/30 flex items-center justify-center">▲</button>
        <div />
        <button onClick={() => changeDirection('LEFT')} className="h-14 w-14 bg-zinc-900 rounded-xl border border-blue-500/30 flex items-center justify-center">◀</button>
        <button onClick={() => changeDirection('DOWN')} className="h-14 w-14 bg-zinc-900 rounded-xl border border-blue-500/30 flex items-center justify-center">▼</button>
        <button onClick={() => changeDirection('RIGHT')} className="h-14 w-14 bg-zinc-900 rounded-xl border border-blue-500/30 flex items-center justify-center">▶</button>
      </div>
    </div>
  );
}