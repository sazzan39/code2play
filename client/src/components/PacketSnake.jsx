  ...
  useEffect(() => {
    ...
    const drawGame = (ctx) => {
      ...
      if (directionRef.current.x === 0 && directionRef.current.y === 0) {
        ...
        ctx.fillText("AWAITING_INPUT_FOR_BREACH", CANVAS_SIZE / 2, CANVAS_SIZE - 20);
      }
    };
  ...
  return (
    <div className="flex flex-col items-center bg-zinc-950 p-4 sm:p-8 rounded-[2rem] border border-blue-500/20 shadow-2xl max-w-full">
      <div className="w-full flex justify-between mb-4 font-mono text-[10px] text-blue-400 uppercase">
        <span>Phase_03 // Packet_Sniffer</span>
        <span className="bg-blue-500/10 px-2 py-1 rounded-full border border-blue-500/20">Intercepts: {score}</span>
      </div>
      
      <div className="relative p-1 bg-gradient-to-br from-blue-500/20 to-transparent rounded-xl">
        <canvas 
          ref={canvasRef} 
          width={CANVAS_SIZE} 
          height={CANVAS_SIZE} 
          className="bg-black rounded-lg border border-white/5 shadow-inner" 
        />
      </div>
      ...
      <div className="mt-8 grid grid-cols-3 gap-3 sm:hidden">
        <div />
        <button onClick={() => changeDirection('UP')} className="h-14 w-14 bg-zinc-900 rounded-xl border border-blue-500/30 flex items-center justify-center active:scale-95 transition-transform">▲</button>
        <div />
        <button onClick={() => changeDirection('LEFT')} className="h-14 w-14 bg-zinc-900 rounded-xl border border-blue-500/30 flex items-center justify-center active:scale-95 transition-transform">◀</button>
        <button onClick={() => changeDirection('DOWN')} className="h-14 w-14 bg-zinc-900 rounded-xl border border-blue-500/30 flex items-center justify-center active:scale-95 transition-transform">▼</button>
        <button onClick={() => changeDirection('RIGHT')} className="h-14 w-14 bg-zinc-900 rounded-xl border border-blue-500/30 flex items-center justify-center active:scale-95 transition-transform">▶</button>
      </div>

      <p className="mt-6 text-[8px] text-zinc-600 uppercase font-mono tracking-widest hidden sm:block">
        Navigate through the firewall to collect encrypted packets
      </p>
    </div>
  );
}